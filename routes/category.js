...
// Register new user
    ...
    router.post("/create-category", async (req, res) => {
        ...
        if(!
         ...
         gender === undefined
            ...
            error.push({
                error_message:
                    "Selecione um gênero para a categoria!"
            });
        }
    ...
    router.get("/create-subcategory", async (req, res) => {
        await Category.find({}).sort({ name: "asc" }).then((categories) => {
            res.render("category/create-subcategory", { categories:categories });
        }).catch((err) => {
            console.log(err);
    ...
    router.post("/create-subcategory", async (req, res) => {
        ...
        if(!category || category === undefined || typeof category === null || category === "") {
            error.push({ error_message: "Por favor, selecione uma categoria pai."})
        }
        ...
        if (error.length > 0) {
            ...
            await Subcategory.findOne({ name:name })
                .then(async subcategory => {
                    if (subcategory) {
                        error.push({ error_message: "Subcategoria já cadastrada!" });
                        console.log(error);
                        res.redirect("/category/create-subcategory");
                        ...
                        await newSubcategory.save().then(() => {
                            console.log(`Subcategoria "${newSubcategory.name}" criada!`);
                        });
